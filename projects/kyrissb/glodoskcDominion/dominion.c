...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
 ...
 //      discardCard(handPos, currentPlayer, state, 0);
    ...
    case steward:
      stewardEffect(choice1, currentPlayer, state, choice2, choice3, handPos);
 //      if (choice1 == 1)
	// {
	//   //+2 cards
	//   drawCard(currentPlayer, state);
...
void adventurerEffect(int drawntreasure, struct gameState * state, int currentPlayer, int cardDrawn, int temphand[], int z)
{
  while (drawntreasure < 2)
  {
    ...
    //Bug originally added, but fixed during unit testing to avoid seg fault  
    while ((z - 1) >= 0) //BUG ADDED:  original line: while ((z - 1) >= 0)
    {
    	state->discard[currentPlayer][state->discardCount[currentPlayer]++] = temphand[z - 1]; // discard all cards in play that have been drawn
    	z = z - 1;
    }
...
{
  //+3 Cards
  for (i = 0; i <= 3; i++) //BUG ADDED: original line: for (i = 0; i < 3; i++)
  {
      drawCard(currentPlayer, state);
  }
...
void villageEffect(int currentPlayer, struct gameState * state, int handPos)
{
  ...
  //discard played card from hand
  discardCard(handPos, currentPlayer, state, 1); //BUG ADDED: original line: discardCard(handPos, currentPlayer, state, 0);
}
...
void great_hallEffect(int currentPlayer, struct gameState * state, int handPos)
{
  ...
  //+1 Actions
  state->numActions--; //BUG ADDED: original line: state->numActions++;
...
//Steward
void stewardEffect(int choice1, int currentPlayer, struct gameState * state, int choice2, int choice3, int handPos)
{
  if (choice1 == 1)
  {
    //+2 cards
